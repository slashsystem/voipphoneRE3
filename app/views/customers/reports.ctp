							...
							<select name="customer" style="width:351px;"  onchange="javascript:submi_form('filters');">
								<option value="GLOBAL" <?php if ($customer_val == 'GLOBAL'){ echo ' SELECTED';}?>><?php __("Global Statistics") ?></option>
									<?php foreach($customerIds  as $customerId){?>
										<option value="<?php echo $customerId['Customer']['id'];?>"<?php if ($customer_val == $customerId['Customer']['id']){ echo ' SELECTED';}?>><column><?php echo $customerId['Customer']['id'];?></column>&nbsp:&nbsp<column><?php echo $customerId['Customer']['name'];?></column></option>
									<?php }?>
									</select>
								</td>
