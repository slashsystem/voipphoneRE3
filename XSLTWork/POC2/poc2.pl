...
my ($inContent,$allFeatures,$newFeature,$featureName,$fullNewFeature,@keyset,@notKeyset);
...
if ($inContent=~/\<Station id="([^\"]*)"/ms){$station=$1;}
...
while ($existingFeatures =~ /(<feature id=\"([^\"]*)\".*?<\/feature>)/msg)
{
	...
	$currentName=~s/[^@]*@?[^\-]*\-//; 	
	if ($con=~/(<feature name="$currentName" defaultKey="[^\"]*" priority="[^\"]*" shiftable="[^\"]*" coloc="[^\"]*" defaultValue="[^\"]*"\/>)/)
	{	
...
sub assignkey()
{
	...
	if ($con =~ /<feature name="$newF" defaultKey="([^\"]*)" priority="([^\"]*)" shiftable="([^\"]*)" coloc="([^\"]*)" defaultValue="([^\"]*)"\/>/ms) #getting various values like priority, shiftable, colocatable from config file for the new feature
	{	
		...
		{	
			...
			{ #new feature with preferred key	
				...
				{	
					...
					$newFFull =~ s/<feature id="([^\-]*\-$newF)" key=\"[^\"]*\" status="99"/<feature id="$desiredkey@\1" key="$desiredkey"/;
					$fullParent .= "\n$newFFull"; 				
			...
			{	
				...
				{	
					if (length($desiredkey)==1){$desiredkey="0$desiredkey";}
					$newFFull =~ s/<feature id="([^\-]*\-$newF)" key=\"[^\"]*\" status="99"/<feature id="$desiredkey@\1" key="$desiredkey"/ms;
					$fullParent .= "\n$newFFull"; 
					$exitFlag="y";
					$index = grep { $notkeyset[$_] eq $desiredkey } 0..$#notkeyset;  splice(@notKeyset, $index, 1);
					$exitFlag="y";					
		...
		{	
			...
			else
			{
			  ...
			  $newFFull =~ s/@/#/;
			  $newFFull =~ s/<feature id="([^\-]*\-$newF)" key=\"[^\"]*\" status="99"/<feature id="$nnDK@\1" key="$nnDK"/;
			  $fullParent .= "\n$newFFull"; 
		...
		{	
			...
			{	
				...
				{	
					...
					{	
						$extF = $1;
						if($con =~ /<feature name="$extF" defaultKey="([^\"]*)" priority="([^\"]*)" shiftable="([^\"]*)" coloc="([^\"]*)" defaultValue="([^\"]*)"\/>/ms)
						{$eP=$2; $eS=$3; $eC=$4;} #getting value of priority etc for current featue from config.xml e stand for existing so eP is existing priority.							
						if ($newF ne $extF)	
						{	
							...
							{	
								...
								$newFFull =~ s/<feature id="([^\-]*\-$newF)" key=\"[^\"]*\" status="99"/<feature id="$key@\1" key="$kkey"/;						
								$exitFlag = "y";
							}							
							...
							{	
								...
								{
									$fullParent =~ s/<feature id="([^\-]*\-$newF)" key=\"[^\"]*\" status="99"/<feature id="$kkey@\1" key="$kkey"/;
								}						
								else
								{	
									$newFFull =~ s/<feature id="([^\-]*\-$newF)" key=\"[^\"]*\" status="99"/<feature id="$kkey@\1" key="$kkey"/;
									$fullParent .= "\n$newFFull"; 
								}						
				...
				#Assigning new key to leftout feature delete $notKeyset[0];
				...
				{
					$fullParent =~ s/<feature id="([^\"]*)" key=\"[^\"]*\" status="99"/<feature id="$llk@\1" key="$llk"/;
					push(@keyArray,$lk); @keyArray = sort @keyArray;
				}
				else
				{	
					$newFFull =~ s/<feature id=\"([^\-]*\-$newF)\" key=\"[^\"]*\" status="99"/<feature id="$llk@\1" key="$llk"/;
					$fullParent .= "\n$newFFull";
					push(@keyArray,"$lk"); @keyArray = sort @keyArray;
...
{ 
	...
	{		
		...
		while ($con =~ /<cfeature name="([^\"]*)" defaultValue="([^\"]*)" mandatory="y" parent="$featureName"\/>/g)
...
{ 
	...
	{		
		...
		{
			if ($con =~ /<feature name="$featureName" defaultKey="[^\"]*" priority="[^\"]*" shiftable="[yn]" coloc="[yn]" defaultValue="([^\"]*)"\/>/g)
			 {	$defval=$1;  
...
sub prettyPrint
{
	...
	my $document = XML::LibXML->new->parse_file('stationOut_originalFormat.xml');
	...
	my $document = XML::LibXML->new->parse_file('stationOut.xml');
	my $pp = XML::LibXML::PrettyPrint->new(indent_string => "  ");
	$pp->pretty_print($document); # modified in-place
