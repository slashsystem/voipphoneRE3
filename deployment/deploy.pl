...
open (LAYOUT, '>>', '/var/www/html/voipphoneRE3/app/views/layouts/default.ctp');
$dateTime = DateTime->now();
#$dateString = $dateTime->strftime('%Y%M%D');
$dateString = $dateTime->datetime();
print LAYOUT 'Build : ' . $dateString;
close LAYOUT;
...
if ($deploymode eq 'REMOTE')
{
	...
	system("cp -r /var/www/html/voipphonebaseconfig/2BPopulator_v2.pl /var/www/html/voipphoneRE3HE5/XSLTWork/POC2/2BPopulator_v2.pl");
	#tar and zip
	...
	system("ssh 176.28.15.224  chmod 777 /var/www/html/voipphoneRE3/app/tmp/logs/*");
	system("ssh 176.28.15.224  chmod 777 /var/www/html/voipphoneRE3/XSLTWork/POC2/*");
}
