...
function validate_form (){
	...
	$('#StationEditForm input.numeric_check[type=text]').each(function(n,element){ 
		...
		if(select_box=='AUD'){
			if(text_input && (text_input.length<1 || text_input.length>30)){
				err_string	+=	UTF8.decode(msg_lang['digits only (0-9) between 1 and 30 digits for'])+key+"<br>";
	...
	$('#StationEditForm input.display[type=text]').each(function(n,element){ 
		...
		var cfdvt =	'data['+key_display+'-CFDVT]';
		var cfdvt_val = $('input[name='+cfdvt+']').val();
		
		
		
		if(cfdvt_val){
			if(cfdvt_val!=''){
			if(!cfdvt_val.match('^([0-9]*)$')){
				err_string	+=	"Value must be numeric for "+key_display;
				error=1;
			}else if(cfdvt_val < 12 || cfdvt_val > 60){
						err_string	+=	" CFDVT Value only greater than 12 and less than 60 is allowed for "+key_display;
						error=1;
		...
		if(display!=''){
			if((display.length>12)){
					err_string	+=	UTF8.decode(msg_lang['maximum 12 digits for Display'])+key_display+"<br>";
					error=1;
			}else{
	...
	$('#StationEditForm input.display[type=text]').each(function(n,element){ 
		...
		var key1			=	'key'+element.id;
		var key_display 	=	$('#'+key1).val();
		var cfb =	'data['+key_display+'-CFB]';
		var cfb_val = $('input[name='+cfb+']').val();
		var cfu =	'data['+key_display+'-CFU]';
		var cfu_val = $('input[name='+cfu+']').val();
		var cfna =	'data['+key_display+'-CFNA]';
		var cfna_val = $('input[name='+cfna+']').val();
			
			if (cfb_val)
			{	
				if(cfb_val!=''){
					if(!cfb_val.match('^([0-9]*)$')){
						err_string	+=	"CBF Value must be numeric for "+key_display;
						error=1;
					}else if(cfb_val.length > 30){
								err_string	+=" CBF Number length must not be greater than 30 digits for "+key_display;
								error=1;
					}
				}
			}
			...
			{
				if(cfu_val != ''){
					if(!cfu_val.match('^([0-9]*)$')){
						err_string	+=	" CFU Value must be numeric for "+key_display;
						error=1;
					}else if(cfu_val.length > 30){
							err_string	+="CBF Number length must not be greater than 30 digits for "+key_display;
							error=1;
					}
				}
			...
			{
				if(cfna_val != ''){
					if(!cfna_val.match('^([0-9]*)$')){
						err_string	+=	" CFNA Value must be numeric for "+key_display;
						error=1;
					}else if(cfna_val.length > 30){
							err_string	+=" CFNA Number length must not be greater than 30 digits for "+key_display;
							error=1;
					}
				}
			}
	...
	if($('#password_type').val()!='ANLG'){
		var password	=	$('#password').val();
			...
			if(password.length<4 || password.length>6){
				...
				err_string	+=	UTF8.decode(msg_lang['Password must have greater than 4 digits and less than 6 digits']);
				error=1;
			}
...
$('.onclick').keydown(function(e){
 ...
 function logview (id){
 	var dest_url =	ajax_url+'logs/logdetails/'+id;	
 	$.fn.colorbox({
				href:dest_url,
				transition:'fade',
				overlayClose:false, 
 ...
 function scriptview (id){
	$.ajaxSetup({ cache: false });
 	var dest_url =	ajax_url+'scenarios/scriptdetails/'+id;	
 	$.fn.colorbox({
				href:dest_url,
				transition:'fade',
				overlayClose:false, 
...
/** recent added js **/
 jQuery(document).ready(function(){ 
	//$(".opencolorbox").colorbox({width:'65%'});
 });