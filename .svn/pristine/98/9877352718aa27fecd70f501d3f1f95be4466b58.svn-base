...
my $inputFileName = "activation.1400105574.input.xml";
my $outputFileName = '';
my $layoutFile = "CICMconfig.xml";
...
if($mode eq 'feature')
{
    		...
    		{	my $noassign = 'no'; my $noassignkey = "";
    			...
    			{	
    				...
    				{	
                        ...
                        #else { $noassign = 'yes'; $noassignkey = $kkey;}						
	...
	#function to check, output of assignkey is correct
	sub checkkeyassign()
	{
	   my $newfp = $fullParent;
	   my $rname =""; my $rfeature = ""; my $rkey=""; my @keys = ""; my $each = ""; my $exprior = ""; my $newprior = ""; my $newname = ""; my $newid = ""; my $exid = "";	   
	   #arranging the keys of feature present in output in descending order
	   ...
	   while($fullParent =~ /(<feature id=\"([^\"]*)\" key="(.*?)".*?<\/feature>)/msg)
       {
		   ...
		   $rname =~ s/.*-(.*)/$1/;
		   #extracting the priority of current feature
		   if ($con =~ /<d?feature name="$rname" short_name="([^\"]*)" type="(key)" mandatory="([^\"]*)" defaultKey="([^\"]*)" priority="([^\"]*)" shiftable="([^\"]*)" coloc="([^\"]*)" defaultValue="([^\"]*)".*?\/>/ms) #getting various values like priority, shiftable, colocatable from config file for the new feature
		   {		 
		      $exprior = $5;              		  
		   }
		   #looping from 1 to greatest key assigned
		   foreach $each (1..$keys[0])
           {    
		        #formatting the key
			    if (length($each)==1){$each = "0$each";} else {}
				#getting details of the feature at the current key
				if($fullParent =~ /(<feature id=\"([^\"]*)\" key="$each".*?<\/feature>)/ms)
				{
				   my $swap = "";
				   $newid = $2;
		           $newname = $2;
    			   #extracting the name of feature
	     		   $newname =~ s/.*-(.*)/$1/;
				   if ($con =~ /<d?feature name="$newname" short_name="([^\"]*)" type="(key)" mandatory="([^\"]*)" defaultKey="([^\"]*)" priority="([^\"]*)" shiftable="([^\"]*)" coloc="([^\"]*)" defaultValue="([^\"]*)".*?\/>/ms) #getting various values like priority, shiftable, colocatable from config file for the new feature
				   {		 
				      #extracting priority of this feature now
					  $newprior = $5;
					  
					  #checking if priority is higher but the key is assigned is greater than the existing feature
                      if($exprior != '' and ($exprior < $newprior) and ($each > $rkey))
					   {
					     
						 $newid =~ s/.*(@.*)/$rkey$1/;
						 $exid =~ s/.*(@.*)/$each$1/;
						 while($newfp =~ /<feature id=\"([^\"]*-$newname)\" key="(.*?)"(.*?)<\/feature>/msg)
						 {   
						    if($2 <= $rkey)
							{
								$swap = 'no';
						 ...
						 #if condition successful then swap the the keys value within these two features						 
						 if($swap =~ /no/){ last;}
						 elsif($type eq 'station'){last;}
						 else
						 {
						 print $rname."    ".$type."    ".$exprior."    ".$rkey."   ".$newname."   ".$newprior."\n";						  
						 $fullParent =~ s/<feature id=\"($each@[^\"]*-$newname)\" key="$each"(.*?)<\/feature>/<feature id="$newid" key="$rkey"$2<\/feature>/ms;
						 $fullParent =~ s/<feature id=\"($rkey@[^\"]*-$rname)\" key="$rkey"(.*?)<\/feature>/<feature id="$exid" key="$each"$2<\/feature>/ms;					 
						 last;
						 }						 
					   }					  
				   }
	...
	{   
		...
		my $arrcount = 0; my $loop = ""; my $nwame = ""; my $nwpr = ""; my $nwky = "";
		 ...
		 while($fullParent =~ /(<feature id=\"([^\"]*)\" key="(.*?)".*?<\/feature>)/msg)
		 {
		   ...
		   if ($con =~ /<d?feature name="$rname" short_name="([^\"]*)" type="([^\"]*)" mandatory="([^\"]*)" defaultKey="([^\"]*)" priority="([^\"]*)" shiftable="([^\"]*)" coloc="([^\"]*)" defaultValue="([^\"]*)".*?\/>/ms) #getting various values like priority, shiftable, colocatable from config file for the new feature
		   {		 
		        push(@rpriority,"$5-$rname-$2"); @rpriority = reverse sort @rpriority;				
                				
		   }		   
		 }
		  #keeping uniq priority
		  my @rpriority1 = uniq(@rpriority);
		  
		 foreach $loop (@rpriority1)
         {
		     if($loop =~ /(.*)-(.*)-(.*)/)
             {  
			    #selecting priority
				...
				#selecting name
				$nwame = $2;
				#selecting type of feature
				$nwky = $3;
             } 		
	        #selecting features of the respective priority
		    while($fullParent =~ /(<feature id=\"([^\"]*)\" key="(.*?)".*?<\/feature>)/msg)
		    {   
			   $rfeature = $1;
			   my $rname1 = $2;
			   #extracting the name of feature
			   $rname1 =~ s/.*-(.*)/$1/;
			    #selecting primary value of feature
			    if($rfeature =~ /\<primary_value\>(.*?)\<\/primary_value>/)
			   {
				$rprimary = $1;
			   }
			   if($rname1 eq $nwame)
			   {
			    #making a sting as unique identifier(priority-name-primary_value-type_of_feature)
				#overwriting the previous values of @priority
			    $rpriority[$arrcount] = $nwpr.'-'.$nwame.'-'.$rprimary.'-'.$nwky;
				$arrcount++;
			   }			   
		    }
		 }		 
		 ...
		 foreach $rassign (@rpriority)
		 {
			...
			if($rassign =~ /.*-(.*)-(.*)-(.*)/)
			{
			    #selecting name of feature from the string stored in @priority
				$rnewname = $1;
			}
		    ...
		    while($rfullParent =~ /(<feature id=\"([^\"]*)\" key="(.*?)".*?<\/feature>)/msg)
			{
				...
				#checking if the pattern matches the current string of the @priority's current index value
				if($rassign =~ /.*-$rnname-$rnprimary-key/)
				{
            
				    my $rncount = "";
					#creating new key from counter
